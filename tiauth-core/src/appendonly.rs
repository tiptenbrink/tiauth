...
fn get_from_elements<T>(elements: &Arc<[OnceLock<T>]>, i: usize) -> Option<&T> {
    elements.get(i).and_then(|e| e.get())
}
...
impl<K, V> AppendOnlyArcMap<K, V> {
        ...
        K: Eq + Hash + Clone,
    {
        match search(&self.state, &self.map, self.map.capacity, key.borrow()) {
            Ok(target) => return Ok(Some(&self.map.get(target).unwrap().1)),
            Err(index) => {
    ...
    where
        ...
        Q: Eq + Hash + ?Sized,
    {
        search(&self.state, &self.map, self.map.capacity, key)
            .ok()
            .map(|index| &self.map.get(index).unwrap().1)
    }
}

fn search<K, Q, V, M: ReadableVector<(K, V)>>(
    state: &RandomState,
    map: &M,
    capacity: usize,
    key: &Q,
...
impl<K, V> MapView<K, V> {
    ...
    where
        ...
        Q: Eq + Hash + ?Sized,
    {
        search(&self.state, &self.map, self.capacity, key)
            .ok()
            .map(|index| &self.map.get(index).unwrap().1)
    }
}
