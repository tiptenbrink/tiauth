...
impl<S: GovernorState> GovernorServerState<S> {
    ...
    pub fn load_application(&mut self, state: S) {
        let previous = self
            .states
            .insert(state.application().to_owned(), RwLock::new(state))
            .unwrap();
        if previous.is_some() {
    ...
    pub fn app_mut_blocking<T, F: FnOnce(&mut S) -> T>(
        &self,
        application: &str,
        f: F,
    ...
    pub async fn app_mut<T: Send + 'static, F: FnOnce(&mut S) -> T + Send + 'static>(
        self,
        application: String,
        f: F,
    ) -> Result<T, ApplicationNotFound> {
